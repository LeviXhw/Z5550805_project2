...
def download_prc_to_csv(start, end, tickers=cfg.TICKERS):
    ...
    for ticker in tickers:
        ...
        stock_data.index.name = 'Date' # change index name, previously called 'price'
        stock_data.reset_index(inplace=True) # reset all index, so that Date can be seen
        stock_data.columns = stock_data.columns.get_level_values(0) # pick the first layer of columns
        data = stock_data[['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume']] # set the desiered columns
        util.color_print(f"Saved {ticker} data to {f"{ticker.lower()}_prc.csv"}") # show the save process
        data.to_csv(os.path.join(cfg.DATADIR, ticker.lower() + '_prc.csv'), index=False)
...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    # 提取指定列数据并去除缺失值
    ser = df_filtered[prc_col].copy().dropna()
    # 过滤出指定日期范围的数据
...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    # 转换为数值类型（浮点数），若有无法转换的值会设为 NaN
    prc = pd.to_numeric(prc, errors='coerce')

    daily_ret = prc.pct_change().dropna()
    daily_ret.name = prc.name
    return daily_ret
...
def monthly_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    # 按月重采样，计算每个月的记录数量、第一天与最后一天的价格
    monthly_count = prc.resample('M').count()
    monthly_first = prc.resample('M').first()
    monthly_last = prc.resample('M').last()

    # 计算月收益率
    monthly_ret = monthly_last / monthly_first - 1

    # 如果某个月记录数少于 18 个交易日，则该月收益率设为 NaN
    monthly_ret[monthly_count < 18] = np.nan

    # 移除所有缺失值的月份
    monthly_ret = monthly_ret.dropna()

    # 将索引转换为 PeriodIndex（月份）
    monthly_ret.index = monthly_ret.index.to_period('M')
    monthly_ret.index.name = 'Year_Month'

    # 保持 Series 名称和原始一致
    monthly_ret.name = prc.name
...
def aj_ret_dict(tickers, start, end):
    ...
    for tic in tickers:
        # 读取价格数据
        ser = read_prc_csv(tic, start, end)
        # 计算日收益率
        dr = daily_return_cal(ser)
        # 计算月收益率
        mr = monthly_return_cal(ser)
        # 存入字典，使用股票代码小写作为列名
        daily_dict[tic.lower()] = dr
        monthly_dict[tic.lower()] = mr

    # 合并各股票的日收益率与月收益率
    daily_df = pd.concat(daily_dict, axis=1)
    monthly_df = pd.concat(monthly_dict, axis=1)
...
def _test_download_prc_to_csv():
    ...
    """
    tickers = ['AAPL', 'TSLA']
    download_prc_to_csv(start='2000-01-01', end='2025-04-12', tickers=tickers)
...
def _test_read_prc_csv():
    ...
    """
    tic = 'AAPL'
    ser = read_prc_csv(tic, '2025-01-04', '2025-04-05')
    util.test_print(ser)
...
if __name__ == "__main__":
    ...
    # ser_price = read_prc_csv(tic='AAPL', start='2025-04-01', end='2025-04-11')
    # _test_daily_return_cal(made_up_data=False, ser_prc=ser_price)
    #
    # # use made-up series to test daily_return_cal function
    # _test_monthly_return_cal()
    # # use apple prc series to test daily_return_cal function
    #ser_price = read_prc_csv(tic='AAPL', start='2020-08-30', end='2021-01-10')
    #print(ser_price)
    # _test_monthly_return_cal(made_up_data=False, ser_prc=ser_price)
