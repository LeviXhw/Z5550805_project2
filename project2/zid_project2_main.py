...
from project2.zid_project2_characteristics import vol_cal
...
def t_stat(df):
    """ Calculating the mean, t-stat and observation counts for the total volatility long-short portfolio.

    Parameters
    ----------
    df : DataFrame
         DataFrame containing 'ls' columns portfolio returns

    Returns
    -------
    DataFrame contains one row and three columns
    1.ls_bar, the mean of 'ls' columns, with 4 decimal points
    2.ls_t, the t stat of 'ls' columns, with 4 decimal points
    3.n_obs, the number of observations of 'ls' columns, as integer

    """

    try:
        # Drop missing values and extract 'ls' column
        ls_returns = df['ls'].dropna().values
        # Count the number of observations
        n = len(ls_returns)
        # Calculate the sample mean
        mean = np.mean(ls_returns)
        # Calculate the sample standard deviation
        std = np.std(ls_returns, ddof=1) if n > 1 else np.nan
        # Calculate the t-statistic
        t_value = mean / (std / np.sqrt(n)) if n > 1 else np.nan

        # Construct and return a DataFrame with the results
        return pd.DataFrame(
            np.array([[round(mean, 4), round(t_value, 4), int(n)]]),
            columns=['ls_bar', 'ls_t', 'n_obs'],
            index=['ls']
        )
...
if __name__ == "__main__":
 ...
 #Q3
 highest_return_2019=get_avg(DM_Ret_dict['Monthly'],2019).idxmax()
 print(f" The highest average monthly return for 2019 is:{highest_return_2019}")

 #Q4
 monthly_return_2019=get_avg(DM_Ret_dict['Monthly'],2019).max()
 print(f" The highest average monthly return for 2019 is:{monthly_return_2019}")

 #Q5
 invest_universe = sorted([col for col in Vol_Ret_mrg_df.columns if '_vol' not in col])
 if len(invest_universe) < 10:
     raise ValueError('investment universe has less then ten stocks')
 number_ten_stock = invest_universe[9]
 vol_col = number_ten_stock + '_vol'

 try:
     vol_df = Vol_Ret_mrg_df[[vol_col]].copy
     avg_vol = get_avg(vol_df,2010)

     print(f"the tenth stock in 2010 is ({number_ten_stock}) and average monthly volatility is:{avg_vol.iloc[0]}")
 ...
 print(f"ls_bar is:{ls_bar}")
 print(f"ls_t is:{ls_t}")
