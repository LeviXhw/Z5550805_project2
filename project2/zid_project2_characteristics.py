...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    """
    # Check whether 'ret' data is input or not
    if ret is None:
        raise ValueError('input must include ret')
    # Check that 'ret' contains 'Daily' and 'Monthly'
    if not all(j in ret for j in ['Daily','Monthly']):
        raise ValueError('input must include daily and monthly')
    # Check if 'Daily' return data is used
    if 'Daily' not in ret_freq_use:
        raise ValueError('ret_freq_use must include daily return data')
    # Create a copy of the daily returns DataFrame to avoid modifying the original data
    daily_ret = ret['Daily'].copy()
    # Create an empty DataFrame to store the calculated volatility results
    vol_df = pd.DataFrame()

    for column in daily_ret.columns:
    # Group the daily returns for the current stock by month using a PeriodIndex.
        monthly_group = daily_ret[column].groupby(pd.PeriodIndex(daily_ret.index, freq='M'))
    # Compute the monthly volatility (standard deviation) for the stock:
    # If there are at least 18 valid daily return entries in a month, calculate the standard deviation;
    # otherwise, assign NaN.
        monthly_vol = monthly_group.agg(lambda x: x.std() if len(x) >= 18 else np.nan)
    # Add the computed volatility series to vol_df with a formatted column name
        vol_df[f"{column}_{cha_name}"] = monthly_vol
    # Set the index name of the resulting DataFrame to 'Year_Month'
    vol_df.index.name = 'Year_Month'
    # Remove rows where all volatility values are NaN, indicating missing data for that month.
...
def merge_tables(ret, df_cha, cha_name):
    ...
    """
    # Raise an error if `ret` is not provided
    if ret is None:
        raise ValueError('input must include ret')
    # Ensure the 'Monthly' key exists in the input dictionary
    if 'Monthly' not in ret:
        raise ValueError('input must include monthly')
    # Check that df_cha is a pandas DataFrame
    if not isinstance(df_cha, pd.DataFrame):
        raise ValueError('df_cha must be dataframe')

    # Create a copy of the monthly returns to avoid modifying the original
    monthly_return = ret['Monthly'].copy()
    # Create a copy of the characteristics table
    df_cha = df_cha.copy()

    # Build a list of required column names based on cha_name
    required_column = [f"{col.split('_')[0]}_{cha_name}" for col in monthly_return.columns]
    # Identify which of the required columns are missing from df_cha
    missing_column = [col for col in required_column if col not in df_cha.columns]
    # If any required characteristic columns are missing, raise an error
    if missing_column:
        raise ValueError(f"df_chamust include: {missing_column}")

    # Merge the monthly return table with the characteristics table
    ...
    # Select all columns in df_cha that end with the characteristic name
    feature_column = [column for column in df_cha.columns if column.endswith(f"_{cha_name}")]
    # Shift all characteristic values forward by 1 month
    merged_df[feature_column] = merged_df[feature_column].shift(1)

    # Remove duplicate columns that were created by the merge
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    """
    #Sanity check on inputs to ensure that parameters are correct
    vol_input_sanity_check(ret, cha_name, ret_freq_use)

    try:
        # Calculating volatility through daily returns
        df_vol = vol_cal(ret = ret, cha_name = cha_name, ret_freq_use = ret_freq_use)
        # Combine the calculated volatilities
        merged_df = merge_tables(ret = ret, df_cha = df_vol, cha_name = cha_name)
        # An error is reported if the result is empty
        if merged_df.empty:
            raise ValueError('The merged data frame is empty')
...
if __name__ == "__main__":
    ...
    # ret_dict = etl._test_aj_ret_dict(tickers=['AAPL', 'TSLA'], start='2010-05-10', end='2010-08-31')
